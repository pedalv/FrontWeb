...
describe('when fetch random users', function () {
	beforeEach(module(
		'ionic',
		'cache.controllers',
		'cache.services'));
	...
	beforeEach(inject(function($rootScope, $controller, $httpBackend, usersService){
		var scope = $rootScope.$new();
		$httpBackend.expectGET("https://randomuser.me/api/?results=10")
			.respond(userList);			
		localStorage.setItem('usersrandom', JSON.stringify(userList));
		userCtrl = $controller('userCtrl', {$scope: scope});
	}));

	it('should has some users', function(){
		expect(userCtrl.users).toBeDefined();
		expect(userCtrl.users.length).toBe(2);
		expect(userCtrl.users[0].name.first).toBe("özsu");
	});
